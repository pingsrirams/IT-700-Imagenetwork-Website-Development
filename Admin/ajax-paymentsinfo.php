...
$sql.=" FROM `payments` where  `status`= 'success'";
...
$sql.=" FROM `payments` WHERE `id`!='' and `status`= 'success' ";
...
$sqld ="SELECT * FROM `tournament` WHERE `id` ='$row[tour_id]' ";

$tournmen= mysqli_query($con, $sqld) or die(mysqli_error($con));

$tournments = mysqli_fetch_array($tournmen);
	...
	$nestedData[] = $row['inv_rand_no'];

	$nestedData[] = date('d/m/Y',strtotime($row['date_time']));

	$nestedData[] = $userinfo['name'];

$nestedData[] = $tournments['name'];
	$nestedData[] = $row['amount'];

	//$nestedData[] = '<a href="#" data-toggle="modal" data-target="#myModal" onClick="return viewInvoice('.$row['id'].');">View</a>';
