...
$sql.=" FROM `tour_join` where `status`='Active'";



$query=mysqli_query($con, $sql) or die("ajax-joinedlist.php: get employees");
...
if($requestData['action'] == 'getsearch') {
	...
	$sql.=($requestData['from_date'] != null && $requestData['to_date'] != null)?" AND `tour_join_starts` BETWEEN '".$requestData['from_date']."' and  '".$requestData['to_date']."' ":"";
...
/*
 $nestedData[] = $checkboxhtml;*/
	...
	$nestedData[] = $tour_access2["name"];

	$nestedData[] = $user_access2["name"];
	$nestedData[] = $user_access2["mobile"];


	$nestedData[] = $tour_access2["fee_type"];

	$nestedData[] = $tour_access2["entry_amount"];

	$nestedData[] = $tour_access2["game_type"];



	$nestedData[] = $tour_access2["tournament_starts"];
...
*/
	...
	$nestedData[] = '<span ><a href="https://www.imnet.imsriram.com/secure_admin/playerinfo/'.$row[id].'" target="blank"><i class="ti-eye color-default"></i></a> </span>';
